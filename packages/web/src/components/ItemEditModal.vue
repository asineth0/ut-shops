...
import { watch, ref, Ref, PropType } from "vue";
import { fileToBase64, prettyError } from "../global/helpers";
import ModalError from "./ModalError.vue";
import PlusIcon from "../icons/PlusIcon.vue";
import { store } from "../global/store";
import { IItem, ITag } from "../global/types";
...
watch(
  ...
  () => {
    ...
    price.value = props.item.price.toString();
    image.value = null;
    tag.value =
      store.state.value.tags.find((tag2) => tag2.id === props.item.tagId) ||
      null;
...
const submit = async () => {
  try {
    ...
    await axios.post(`/api/items/${props.item.id}`, {
      tagId: tag.value.id,
      name: name.value,
